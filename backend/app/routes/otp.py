...
def send_email(to, otp):
    ...
    if not all((host,user,password,sender)): return {'sent':False,'configured':False,'message':'Email OTP is not configured. Add SMTP_HOST, SMTP_USERNAME, SMTP_PASSWORD and SMTP_FROM.'}
    msg=EmailMessage(); msg['Subject']='FarmerSetu verification code'; msg['From']=sender; msg['To']=to
    msg.set_content(f'Your FarmerSetu OTP is {otp}. It expires in 5 minutes. Do not share this code with anyone.')
    try:
...
def request_otp(payload:OTPRequest,db:Session=Depends(get_db)):
    ...
    _pending[key]={'otp':otp,'expires':now+timedelta(minutes=5),'sent_at':now,'attempts':0,'user_id':user.id}
    ...
    if not result.get('sent'): _pending.pop(key,None); raise HTTPException(502,result.get('message','Unable to send OTP.'))
...
def verify_otp(payload:OTPVerify,db:Session=Depends(get_db)):
    ...
    target=user.mobile if payload.channel=='sms' else user.email
    key=f'{user.id}:{payload.channel}:{target}'
