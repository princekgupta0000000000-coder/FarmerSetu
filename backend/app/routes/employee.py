...
def update_booking(booking_id: str,data: BookingUpdate,_: User=Depends(current_employee),db: Session=Depends(get_db)):
    ...
    if not b: raise HTTPException(404,'Booking not found')
    v=data.model_dump(exclude_none=True);old_status,old_quality,old_payment=b.status,b.quality_status,b.payment_status
    ...
    if received is not None:
        if received<=0: raise HTTPException(400,'Received quantity must be greater than zero')
        if received>b.quantity: raise HTTPException(400,f'Received quantity cannot exceed booked quantity ({b.quantity:g} quintal)')
    target_quality=v.get('quality_status',b.quality_status)
    if target_quality=='Passed' and received is None: received=b.quantity;v['received_quantity']=received
    if target_quality=='Passed' and b.status=='Cancelled': raise HTTPException(400,'Cancelled booking cannot pass quality')
    if target_quality=='Passed' and not b.payment_reference: v['payment_reference']=make_transaction_id()
    target_payment=v.get('payment_status',b.payment_status)
    if target_payment=='Paid':
        if target_quality!='Passed': raise HTTPException(400,'Quality must be passed before payment')
        ...
        if received is None: received=b.quantity;v['received_quantity']=received
    for key,value in v.items():setattr(b,key,value)
    ...
    if old_payment!=b.payment_status:
        if b.payment_status=='Processing':add_notification(db,b,'Payment is processing',f'Payment for booking {b.token} is being processed.','payment')
...
def mark_paid(booking_id: str,_: User=Depends(current_employee),db: Session=Depends(get_db)):
    b=db.scalar(select(Booking).where((Booking.booking_id==booking_id)|(Booking.token==booking_id)))
    if not b: raise HTTPException(404,'Booking not found')
    if b.status=='Cancelled': raise HTTPException(400,'Cancelled booking cannot be paid')
    if b.received_quantity is None:b.received_quantity=b.quantity
    if b.quality_status!='Passed':b.quality_status='Passed'
    if not b.payment_reference:b.payment_reference=make_transaction_id()
...
def paid_legacy(booking_id: str,_: User=Depends(current_employee),db: Session=Depends(get_db)): return mark_paid(booking_id,_,db)
@router.delete('/bookings/{booking_id}')
def delete_booking(booking_id: str,_: User=Depends(current_employee),db: Session=Depends(get_db)): return _delete_booking_by_key(booking_id,db)
@router.post('/bookings/{booking_id}/delete')
def delete_booking_post(booking_id: str,_: User=Depends(current_employee),db: Session=Depends(get_db)): return _delete_booking_by_key(booking_id,db)
